...
const emptyPlan={salableProductId:"",salableProductName:"",name:"",frequency:"monthly" as SubscriptionFrequency,deliveriesPerTerm:4,price:0,deliveryChargeMode:"per_delivery" as const,deliveryCharge:0,active:true,description:""};
...
function PlanModal({value,salableProducts,onClose,onSaved,onError}:{value:SubscriptionPlan|null;salableProducts:SalesProduct[];onClose:()=>void;onSaved:()=>Promise<void>;onError:(s:string)=>void}){
 const[v,setV]=useState(value?{...value,salableProductId:value.salableProductId??"",salableProductName:value.salableProductName??""}:emptyPlan);const[saving,setSaving]=useState(false);
 ...
 return <Modal title={value?"Edit Subscription Plan":"Add Subscription Plan"} close={onClose}><form onSubmit={save}><div className="row g-3">
  <div className="col-md-6"><label className="form-label">Salable Product *</label><select className="form-select" value={v.salableProductId} onChange={e=>{const product=salableProducts.find(x=>x.id===e.target.value);setV({...v,salableProductId:product?.id??"",salableProductName:product?.name??""})}} required><option value="">Select Salable Product</option>{salableProducts.filter(x=>x.active!==false).map(x=><option key={x.id} value={x.id}>{x.name}{x.sku?` (${x.sku})`:""}</option>)}</select></div>

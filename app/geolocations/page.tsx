...
export default function GeolocationsPage() {
  ...
  return <AdminPage><div className="container-fluid py-3">
    <div className="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-3">
      <div><h1 className="h3 seedlings-brand mb-1">Geolocation Master</h1><p className="text-muted mb-0">Manage pincode-wise delivery locations and charges for one-time and subscription orders.</p></div>
      <div className="d-flex gap-2"><button className="btn btn-outline-secondary" onClick={() => void load()} disabled={loading} title="Refresh"><i className="bi bi-arrow-clockwise" /></button><button className="btn btn-success" onClick={openCreate}><i className="bi bi-plus-lg me-1" /> Add Geolocation</button></div>
    </div>
    {error && <div className="alert alert-danger">{error}</div>}

    {showForm && <div className="card border-success mb-3"><div className="card-header"><strong>{editing ? "Edit Geolocation" : "Add Geolocation"}</strong></div><form onSubmit={save}>
      ...
      <div className="card-footer d-flex justify-content-end gap-2"><button type="button" className="btn btn-secondary" onClick={() => setShowForm(false)} disabled={saving}>Cancel</button><button className="btn btn-success" disabled={saving}>{saving ? "Saving..." : editing ? "Update Geolocation" : "Create Geolocation"}</button></div>
    </form></div>}

    <div className="card"><div className="card-header d-flex justify-content-between align-items-center"><div><strong>Geolocation Master</strong><div className="small text-muted">Pincode, location name and delivery charges.</div></div><span className="badge text-bg-secondary">{locations.length}</span></div>
      <div className="table-responsive"><table className="table table-hover align-middle mb-0"><thead><tr><th>Location Name</th><th>Pincode</th><th>One-time Charge</th><th>Subscription Charge</th><th>Status</th><th className="text-end">Actions</th></tr></thead><tbody>
        ...
        {!locations.length && !loading && <tr><td colSpan={6} className="text-center text-muted py-5">No geolocations yet.</td></tr>}
